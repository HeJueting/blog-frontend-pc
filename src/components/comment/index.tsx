						...
						<div className={style["item-wrap"]} key={item._id}>
							...
							<div className={style["img-wrap"]}>
								{headerImgObj[item.name] ? (
									<img src={headerImgObj[item.name]} />
								) : (
									<Icon type="iconuser" className={style["icon"]} />
								)}
							</div>
								...
								{item.quote && (
									<div className={style["reply-content"]}>
										<header className={style["header"]}>
											回复 <span>{getQuoteInfo(item.quote).name}</span> 的评论：
										</header>
										<div
											className={"braft-output-context"}
											dangerouslySetInnerHTML={{
												__html: getQuoteInfo(item.quote).html,
											}}
										></div>
									</div>
								)}
