		...
		<>
			...
			>
				<div className={style["section-one-content"]}>
					...
					<p className={style["section-one-name"]}>{userInfo.nickName}</p>
					<p className={style["section-one-job"]}>{userInfo?.job}</p>
					<p className={style["section-one-motto"]}>{userInfo.motto}</p>
				</div>
			</section>

			{/* 第二屏 */}
			<section className={`${style["home-section"]} ${style["section-two"]}`}>
				...
				<div className={style["section-two-right"]}>
					...
					<div className={style["section-two-right-context"]}>
						...
						<div className={style["context-info"]}>
							...
							/>
							<span style={{ marginRight: "0.8vw" }}>Location：</span>
							<span>{userInfo.city}</span>
						</div>
						<div className={style["context-info"]}>
							<Icon type="icongongsi" className={style["context-info-icon"]} />
							<span style={{ marginRight: "0.8vw" }}>Company：</span>
							<span>{userInfo.company}</span>
						</div>
						<div className={style["context-info"]}>
							<Icon type="iconlove" className={style["context-info-icon"]} />
							<span style={{ marginRight: "0.8vw" }}>Hobbys：</span>
							<p>
								{lodash
									.get(userInfo, "hobby", [])
									.map((item: string, index: number) => (
										<span key={item}>{`${item}${
											index + 1 != lodash.get(userInfo, "hobby.length")
												? "、"
												: ""
										}`}</span>
									))}
						...
						<div className={style["context-info"]}>
							<Icon
								...
								className={style["context-info-icon"]}
							/>
							<span style={{ marginRight: "0.8vw" }}>Labels：</span>
							<p>
								{lodash
									.get(userInfo, "label", [])
									.map((item: string, index: number) => (
										<span key={item}>{`${item}${
											index + 1 != lodash.get(userInfo, "label.length")
												? "、"
												: ""
										}`}</span>
									))}
			...
			<section
				...
				style={{
					backgroundImage: `url(${dealBacImg(settingInfo.firstBacImage)})`,
				}}
			>
						...
						<Link href={"/"} key={item.title}>
							<div className={style["three-wrap"]}>
								...
								<div className={style["three-icon-wrap"]}>
									<Icon type={item.icon} className={style["three-iconfont"]} />
								</div>
								<h5 className={style["three-title"]}>{item.title}</h5>
							</div>
						</Link>
					))}
				</div>
			...
			{/* 第四屏 */}
			<section className={`${style["home-section"]} ${style["section-four"]}`}>
							...
							<Link href={"/"} key={item.title}>
								<div className={style["four-article-item"]}>
									{item.bacImg ? (
										...
										<div className={style["four-icon-wrap"]}>
											<Icon
												type="iconbook"
												className={style["four-iconbook"]}
											/>
										</div>
									)}

									<div className={style["four-mask-wrap"]}>
										<p className={style["four-mask-wrap-title"]}>
											{item.title}
										</p>
									</div>
								</div>
			...
			<section
				...
				style={{
					backgroundImage: `url(${dealBacImg(settingInfo.thirdBacImage)})`,
				}}
			>
				<div className={style["section-five-content"]}>
					<div className={style["suggestion-wrap"]}>
						<h1 className={style["five-context-title"]}>Suggestion</h1>
						<Textarea
							placeholder="请输入您的意见"
					...
					<div className={style["contact-wrap"]}>
						...
						<div className={style["contact-item-wrap"]}>
							<Icon type="iconemail" className={style["contact-iconfont"]} />
